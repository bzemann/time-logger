...
- An "Hours per day" bar chart with a dashed target line. The dashed line
  always stays at the normal `daily_target`; a day with a
  `target_overrides` entry shows its actual (overridden) target in the
  per-day tooltip instead.
...
`worktime dashboard` detects when the background server is running an
older version than the code on disk (by comparing `/api/health`) and
restarts it automatically, printing "Restarted the dashboard server (new
version)." — so after a `git pull`, the dashboard server restarts itself
the next time you open the dashboard; `worktime stop-server` is optional
and mainly useful to stop it without reopening the browser.
...
# Per-date target overrides (half days, special days): a table where each
# key is a single date "YYYY-MM-DD" or an inclusive range
# "YYYY-MM-DD..YYYY-MM-DD" (up to 366 days), and each value is a
# daily-target-style value ("H:MM" or a number of hours, 0-24h). The
# override replaces the normal target for that date, including on
# weekends; "0:00" behaves like a day off. A date can't be in both
# days_off and target_overrides, and a date can't be defined twice. This
# table must come after all the keys above (it's a TOML table).
# [target_overrides]
# "2026-09-24" = "4:15"
# "2026-12-24..2026-12-31" = "0:00"
```
...
| `target_overrides` | `{}` | Table mapping a date or inclusive date range (max 366 days) to a per-date target, replacing the normal target for that date |

Any key that isn't one of the above makes the config file invalid
(`worktime` exits with an error naming the unknown key).

`worktime config` shows the configured overrides on their own line, e.g.
`overrides:    1 day (2026-09-24 = 4:15)`, `overrides:    (none)`, or for
more than one, `overrides:    3 days (2026-09-24 … 2026-09-26)`.
...
- Every configured **workday** that is not a configured **day off** has a
  ...
  have a target of 0.
- A date listed in `target_overrides` uses that target instead, on any
  day of the week (weekends included) — it takes precedence over the
  usual workday/day-off rule. `"0:00"` behaves like a day off. A date
  can't be in both `days_off` and `target_overrides` at once (`worktime`
  refuses to start with such a config). Overrides apply **retroactively**:
  changing one recalculates past balances immediately, but any report
  already marked "final" has to be regenerated by hand (`worktime report
  week|month --date …`) to pick up the change.
...
Example with an override: on a half day off (`target_overrides = {
"2026-09-24" = "4:15" }`), working 4h 15m that day gives a balance of
±0:00 for that day, instead of the −4:15 it would show against the full
`daily_target`.
...
- **The dashboard shows old behaviour after an update:** `worktime
  dashboard` restarts an outdated server automatically; if it somehow
  didn't, run `worktime stop-server` and open the dashboard again.
- **The installer frames:** `install.sh` prints what to paste between
  `──── paste into … ────` / `──── replace this line in … ────` and
  `──── end ────` lines — only copy what's between those lines; the
  sentences around the frame are instructions, not something to paste.
- **polybar says "Unknown line type" (or fails to parse the config):** an
  instruction line (not one of the `──── … ────` framed lines) was pasted
  into `config.ini` by mistake — remove it, keeping only the framed
  `[module/worktime]` block and the `modules-right` line.
- **`sh ~/.config/polybar/launch.sh` fails with "disown: not found":**
  `launch.sh` is a bash script; run it directly
  (`~/.config/polybar/launch.sh`) or with `bash
  ~/.config/polybar/launch.sh`, not `sh`.
...
1. `cd <repo> && git pull`
2. `platform/macos/install.sh` (safe to re-run — it refreshes the
   Spotlight launchers; "already configured" for a key means there's
   nothing new to paste)
3. Paste anything new it prints between `──── … ────` lines, then reload
   Aerospace (`alt-shift-c`).
4. Check `worktime --version`.
5. Read [`CHANGELOG.md`](CHANGELOG.md) for anything version-specific to
   do by hand.

**Debian:**

1. `cd <repo> && git pull`
2. `platform/linux/install.sh` (safe to re-run)
3. Paste anything new it prints between `──── … ────` lines — i3 lines go
   into `~/.config/i3/config` (reload with `$mod+shift+c`); polybar
   changes go into `~/.config/polybar/config.ini` (restart with
   `~/.config/polybar/launch.sh`, not a plain i3 reload).
4. Check `worktime --version`.
5. Read [`CHANGELOG.md`](CHANGELOG.md) for anything version-specific to
   do by hand.
...
- The dashboard server restarts itself the next time you run `worktime
  dashboard` if its version doesn't match the code on disk — running
  `worktime stop-server` after an update is optional, not required.
- An update never touches your data, config or reports — only the code
  in the repository changes.
- If the repository was moved or renamed, re-run the platform install
  script (`platform/macos/install.sh` or `platform/linux/install.sh`) so
  the launchers pick up the new absolute path.
...
CHANGELOG.md             user-facing changelog, newest version first
```

See [`CHANGELOG.md`](CHANGELOG.md) for what changed in each version and
what (if anything) to do by hand after updating.
